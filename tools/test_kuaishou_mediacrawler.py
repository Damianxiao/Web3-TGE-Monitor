...
from crawler.platforms.kuaishou_platform import KuaishouPlatform
...
async def test_kuaishou_availability():
    """测试快手平台是否可用"""
    print("=" * 50)
    print("测试快手平台可用性")
    print("=" * 50)
    ...
    try:
        ...
        config = {
            ...
            'kuaishou_enabled': settings.kuaishou_enabled,
            'kuaishou_login_method': settings.kuaishou_login_method,
            'kuaishou_headless': settings.kuaishou_headless,
        }
        
        platform = KuaishouPlatform(config)
        
        # 测试平台可用性
        is_available = await platform.is_available()
        print(f"快手平台可用性: {'✓ 可用' if is_available else '✗ 不可用'}")
        
        if not is_available:
            print("快手平台不可用，请检查MediaCrawler配置")
            return False
        
        return True
        
    except Exception as e:
        print(f"测试快手平台可用性失败: {e}")
        return False
...
async def test_kuaishou_search():
    ...
    try:
        ...
        print(f"测试快手搜索功能失败: {e}")
        import traceback
        traceback.print_exc()
        return False

async def test_kuaishou_data_structure():
    """测试快手数据结构"""
    print("\n" + "=" * 50)
    print("测试快手数据结构")
    print("=" * 50)
    ...
    try:
        ...
        print("数据结构验证:")
        print(f"✓ 平台类型: {result.platform} (应为 {Platform.KUAISHOU})")
...
async def main():
    """主测试函数"""
    print("快手平台MediaCrawler集成测试")
    print(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"MediaCrawler路径: {settings.mediacrawler_path}")
    print(f"Cookie配置: {'已配置' if settings.kuaishou_cookie else '未配置'}")
    
    test_results = []
    
    # 测试1: 平台可用性
    result1 = await test_kuaishou_availability()
    test_results.append(("平台可用性", result1))
    
    if result1:
        # 测试2: 搜索功能
        result2 = await test_kuaishou_search()
        test_results.append(("搜索功能", result2))
        
        if result2:
            # 测试3: 数据结构
            result3 = await test_kuaishou_data_structure()
    ...
    if all_passed:
        print("\n快手平台MediaCrawler集成测试成功完成！")
        print("可以开始使用快手平台进行数据爬取。")
    else:
        print("\n快手平台MediaCrawler集成测试失败！")
